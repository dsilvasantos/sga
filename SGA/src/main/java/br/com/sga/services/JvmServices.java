...
public class JvmServices {
	...
	public void getServerInformations(Server server) {
		...
		String commandPortOffSet = cmd + "/socket-binding-group="+server.getSocketBindingGroup()+":read-attribute(name=port-offset)";
		...
		if(time<36000000){
			server.setServerUptime(String.format("%01d:%02d",TimeUnit.MILLISECONDS.toHours(time),TimeUnit.MILLISECONDS.toMinutes(time) %60 ));
		}		
		if(time>36000000 && time<360000000){
			server.setServerUptime(String.format("%02d:%02d",TimeUnit.MILLISECONDS.toHours(time),TimeUnit.MILLISECONDS.toMinutes(time) %60 ));
		}
		else if(time>360000000){
			server.setServerUptime(String.format("%03d:%02d",TimeUnit.MILLISECONDS.toHours(time),TimeUnit.MILLISECONDS.toMinutes(time) %60 ));
		}
		...
		for(DeploymentsVersion lista:listDeploys){
			...
			if(lista.getNomeAplicacao().contains(".ear")){
				...
				String web = cmd + "/deployment="+lista.getNomeAplicacao()+"/subsystem=web:read-attribute(name=context-root)";
				lista.setContextoRaiz(service.readAttribute( web));
				lista.setProfile(server.getProfile());
				server.setVersion(listDeploys);
